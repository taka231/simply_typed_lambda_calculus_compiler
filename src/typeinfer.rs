...
pub enum Type {
    ...
    TVar(u32, Rc<RefCell<Option<Type>>>),
    // 以下の型は、closure conversionの時に使う
    Tuple(Vec<Type>),
}
...
impl Type {
    pub fn simplify(&self) -> Self {
        match self {
            ...
            Type::Tuple(ts) => Type::Tuple(ts.iter().map(|t| t.simplify()).collect()),
            _ => self.clone(),
        }
    }
    ...
    pub fn get_type(env: &[Type], expr: &Expr) -> Option<Type> {
        match expr {
            Expr::Var(var) => env.get(var.id.unwrap() as usize).map(|t| t.simplify()),
            Expr::Abs(var, expr) => {
                let t = env.get(var.id.unwrap() as usize)?.simplify();
...
impl TypeInfer {
    ...
    pub fn type_infer(&mut self, expr: &Expr) -> Option<Type> {
        match expr {
            Expr::Var(var) => self.env.get(var.id.unwrap() as usize).map(|t| t.clone()),
            Expr::Abs(var, expr) => {
                let t = self.env.get(var.id.unwrap() as usize)?.clone();
